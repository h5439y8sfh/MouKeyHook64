...
NTSTATUS Keyboard_Create(IN PDRIVER_OBJECT driverObject)
{
	...
	DbgPrintEx( DPFLTR_IHVDRIVER_ID,  DPFLTR_INFO_LEVEL,"Keyboard_Init ----> Keyboard_Init status[%x].\n", status);
...
NTSTATUS Keyboard_Hook(IN PDRIVER_OBJECT driverObject)
{
	...
	DbgPrint("Running Keyboard_Create.");
	...
	DbgPrintEx( DPFLTR_IHVDRIVER_ID,  DPFLTR_INFO_LEVEL,"Keyboard_Hook ----> kbdId[%d].\n", kbdId);
	...
	if(NT_SUCCESS(status))
	{
		...
		DbgPrintEx( DPFLTR_IHVDRIVER_ID,  DPFLTR_INFO_LEVEL,"KeyBoard_Create ----> KeyBoard_HookProc success.\n");
	}
...
NTSTATUS Keyboard_UnHook(IN PDRIVER_OBJECT driverObject)
{
	...
	DbgPrint("Running Keyboard_Close.");
...
NTSTATUS KeyboardApc(void *a1, void *a2, void *a3, void *a4, void *a5)
{
	...
	//DbgPrint("KeyboardApc: max=%x", max);
...
// 	DbgPrint("KeyBoard_HookProc: success");
